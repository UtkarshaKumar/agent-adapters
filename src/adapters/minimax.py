from typing import List
from .base import BaseAgent, Task, AgentResponse, Capability, AgentType
from ..orchestrator.agentops_observer import get_observer, AgentOpsConfig


class MinimaxAgent(BaseAgent):
    """Minimax API wrapper adapter."""

    def __init__(self, api_key: str, base_url: str = "https://api.minimax.chat/v1", 
                 agentops_key: str = None):
        super().__init__(api_key, AgentType.MINIMAX)
        self.base_url = base_url
        self._client = None
        
        observer_config = AgentOpsConfig(api_key=agentops_key, tags=["minimax", "agent-adapter"])
        self.ops_observer = get_observer(observer_config)
        self.ops_observer.initialize()

    def _get_client(self):
        if self._client is None:
            try:
                import requests

                self._client = requests.Session()
                self._client.headers.update({"Authorization": f"Bearer {self.api_key}"})
            except ImportError:
                raise RuntimeError("requests not installed. Run: pip install requests")
        return self._client

    def run(self, task: Task) -> AgentResponse:
        import time
        import requests

        start = time.time()
        self.ops_observer.start_session(tags=["minimax", task.id])

        try:
            client = self._get_client()
            response = client.post(
                f"{self.base_url}/text/chatcompletion_v2",
                json={
                    "model": "Minimax-Text-01",
                    "messages": [{"role": "user", "content": task.prompt}],
                },
                timeout=task.context.get("timeout", 300),
            )
            response.raise_for_status()
            data = response.json()

            elapsed_ms = int((time.time() - start) * 1000)
            output = data.get("choices", [{}])[0].get("message", {}).get("content", "")
            
            usage = data.get("usage", {})
            prompt_tokens = usage.get("prompt_tokens", 0)
            completion_tokens = usage.get("completion_tokens", 0)
            cost = self._estimate_cost(prompt_tokens, completion_tokens)

            self.ops_observer.record_action(
                agent_type="minimax",
                action="chat_completion",
                success=True,
                latency_ms=elapsed_ms,
                cost=cost,
                metadata={
                    "model": "Minimax-Text-01",
                    "task_id": task.id,
                    "prompt_tokens": prompt_tokens,
                    "completion_tokens": completion_tokens
                }
            )
            self.ops_observer.end_session(success=True)

            return AgentResponse(
                success=True,
                output=output,
                error=None,
                metrics={
                    "latency_ms": elapsed_ms,
                    "agent": "minimax",
                    "model": "Minimax-Text-01",
                    "cost": cost,
                },
                task_id=task.id,
            )
        except Exception as e:
            elapsed_ms = int((time.time() - start) * 1000)
            self.ops_observer.record_action(
                agent_type="minimax",
                action="error",
                success=False,
                latency_ms=elapsed_ms,
                metadata={"error": str(e)}
            )
            self.ops_observer.end_session(success=False)
            return AgentResponse(
                success=False,
                output="",
                error=str(e),
                metrics={"latency_ms": elapsed_ms, "agent": "minimax"},
                task_id=task.id,
            )

    def _estimate_cost(self, prompt_tokens: int, completion_tokens: int) -> float:
        INPUT_COST_PER_1K = 0.001
        OUTPUT_COST_PER_1K = 0.003
        return (prompt_tokens / 1000) * INPUT_COST_PER_1K + \
               (completion_tokens / 1000) * OUTPUT_COST_PER_1K

    def get_capabilities(self) -> List[Capability]:
        return [
            Capability(
                "text-generation",
                "Generate text content",
                latency_ms=800,
                supports_streaming=True,
            ),
            Capability("code-generation", "Write code snippets", latency_ms=1500),
            Capability("translation", "Translate text", latency_ms=1000),
        ]
